...
class Bot extends EventEmitter {
    constructor(botid) {
        ...
        this.name = "b" + botid;
        this.botid = botid;
        this.home = path.join(__dirname, "..", "account", "user-accounts", this.name)

        // Create a network namespace for this bot
        if (!USER.SUPPORTS_FJ_NET)
            child_process.execSync("./scripts/network " + this.botid)
    ...
    full_stop() {
        ...
        if (!USER.SUPPORTS_FJ_NET && fs.existsSync(`/var/run/netns/catbotns${this.botid}`))
            child_process.execSync(`./scripts/delete ${this.botid}`)

...
function updateIPCData(row, id, data) {
	...
	var time = Math.floor(Date.now() / 100 - data.heartbeat);
	if (!data.heartbeat || time < 4) {
		...
		row.find('.client-status').removeClass('error').addClass('warning').text('Warning ' + time);
			...
			if ((Date.now() - data.ts_injected * 100 > 20) && data.heartbeat && !autorestart[row.attr('data-id')] || (Date.now() - autorestart[row.attr('data-id')]) > 1000 * 5) {
	...
	if (data.connected) {
		...
		row.find('.client-uptime-server').text(format(Date.now() - data.ts_connected * 100));
		...
		if (data.ts_disconnected) {
			row.find('.client-uptime-queue').text(format(Date.now() - data.ts_disconnected * 100));
		} else {
			row.find('.client-uptime-queue').text(format(Date.now() - data.ts_injected * 100));
		}

...
function commandButtonCallback() {
    var cmdz = prompt('Type a command in input');
    if (cmdz) {
		cmd('exec', {
...
function updateIPCData(row, id, data) {
		...
		row.find('.client-status').removeClass('error').addClass('warning').text('Warning ' + time);
			...
			if ((Date.now() - data.ts_injected * 1000 > 20) && data.heartbeat && !autorestart[row.attr('data-id')] || (Date.now() - autorestart[row.attr('data-id')]) > 1000 * 5) {
			    ...
			    request(`api/bot/${row.attr('data-id')}/restart`, function(e, r, b) {
					if (e) {
						console.log(e,b);
						status.error('Error restarting ' + JSON.stringify(data));
					} else {
						status.info('Bot restarted');
					}
				});
	...
	if (data.connected) {
		...
		row.find('.client-alive').text(data.ingame.life_state ? 'No' : 'Yes');
...
function addClientRow(botid) {
    ...
    actions.append($('<input>').attr('type', 'button').attr('value', 'Command').attr('class', "btn btn-link btn-sm").on('click', commandButtonCallback));
    actions.append($('<input>').attr('type', 'button').attr('value', 'Restart').attr('class', "btn btn-link btn-sm").on('click', restartButtonCallback));
    actions.append($('<input>').attr('type', 'button').attr('value', 'Stop').attr('class', "btn btn-link btn-sm").on('click', terminateButtonCallback));
    row.append(actions);
	row.append($('<td></td>').attr('class', 'client-restarts').text('None'));
	row.append($('<td></td>').attr('class', 'client-state').text('Finding'));
    ...
    row.append($('<td></td>').attr('class', 'client-alive connected active').text('None'));
    ...
    row.append($('<td></td>').attr('class', 'client-players connected active').text('None'));
    $('#clients').append(row);
    return row;
}
...
function refreshComplete() {
	...
	request.get({
		...
		if (e) {
			console.log(e, b);
			status.error('Cant refresh list');
			return;
		}
		var count = 0;
		var b = JSON.parse(b);
